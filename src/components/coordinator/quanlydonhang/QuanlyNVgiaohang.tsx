...
function Row(props: {
  ...
  React.useEffect(() => {
    const fetchLocation = async (location: LongLatData) => {
      ...
      // "Đình Thôn Hạ, Định Công Thượng, Hanoi, 11700, Vietnam" tôi muốn cắt bỏ hết từ sau , 11700, Vietnam
      const parts = resp.split(', ');
      const result = parts.slice(0, -2).join(', ');
      result.replace("Hanoi", "Hà Nội");

      setAddressText(result);
    }
    if (carrier) {
      fetchLocation({
        latitude: carrier.latitudeNewest,
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell align="left">{addressText}</TableCell>
        <TableCell align="left">{carrier.active ? <Chip color={CarrierActiveObject.active.color} label={CarrierActiveObject.active?.label || ""} /> : <Chip color={CarrierActiveObject.inactive.color} label={CarrierActiveObject.inactive.label || ""} />}</TableCell>
        <TableCell align="left">{carrier.available ? <Chip color={CarrierAvailableObject.available.color} label={CarrierAvailableObject.available.label} /> : <Chip color={CarrierAvailableObject.inavailable.color} label={CarrierAvailableObject.inavailable?.label} />}</TableCell>
      </TableRow>
      ...
      <Table aria-label="collapsible table">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="left">Vị trí hiện tại</TableCell>
            <TableCell align="left">Trạng thái hoạt động</TableCell>
            <TableCell align="left">Trạng thái giao hàng</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
